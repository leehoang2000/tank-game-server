...
public class Room{
	...
	private TankGameServer tankGameServer;
	...
	public Room(int roomID, ServerSideSender serverSideSender, TankGameServer tankGameServer) {
		...
		this.roomID = roomID;
		this.serverSideSender = serverSideSender;
		this.tankGameServer = tankGameServer;
		
		//Implement Thread Pooling
		pls = new PingLoopService(Players,serverSideSender);
		
		pls.start();
	}

	public int addSocket(InetSocketAddress newSocket) throws IOException {
	...
	public int getPlayerID(InetSocketAddress socket) {
...
//		}
		for (Entry<InetSocketAddress, Player> entry : Players.entrySet()) {
			if (entry.getValue().getID() != excludeID) {
				serverSideSender.sendRawMessage(entry.getKey(), message);
			}
		}
	}
...
//			System.out.println("Server.RoomManager.broadcastRoomMemberUpdate| send to: " + Players.get(player));
			serverSideSender.sendRoomMemberUpdateMessage(player, getAllPlayerID());
		}
	}
	...
	public void announceGameResult(int teamNumber) {
				...
				for (Entry<InetSocketAddress, Player> entry : Players.entrySet()) {
					try {
						if (team1IDs.contains(entry.getValue().getID())) {
							serverSideSender.sendGameWonMessage(entry.getKey());
						} else {
							serverSideSender.sendGameLostMessage(entry.getKey());
						}
					} catch (IOException e) {
						e.printStackTrace();
					}
		...
		tankGameServer.deleteRoom(this.roomID);
	}
